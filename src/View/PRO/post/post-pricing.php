...
?>
<p><label><input type="checkbox" value="1" id="show-override-pricing" name="override-pricing" <?php if ($overrideChecked) echo "checked='checked'"; ?>/><span data-automatically-manage="<?= $automaticallyManage ?>" data-manually-manage="<?= $manuallyManage ?>"><?= $display ?></span></label></p>
<div id="override-pricing"<?php if ($overrideChecked) echo " style='display:block'"; ?>>
    ...
    <select name="pricing-group-id" id="pricing-group-id">
        ...
        foreach ($pricingGroups as $pricingGroup) {
            $selected = ($pricingGroup->getPricingGroupID() == $pricingGroupSelected ? ' selected="selected"' : '');
